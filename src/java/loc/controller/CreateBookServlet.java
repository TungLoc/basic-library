...
public class CreateBookServlet extends HttpServlet {
    ...
    private static final String ERROR = "createNewBook.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (bookID.trim().isEmpty()) {
                check = true;
                errorDTO.setBookIDError("BookID is not empty");
            }
            ...
            if (check) {
                ...
                if (DBUtils.checkFormat(quantity.trim(), "^[0-9]{1,5}$")) {
                    check = true;
                    errorDTO.setQuantityError("Quantity is a number");
                }
                if (DBUtils.checkFormat(price.trim(), "^[0-9\\.]{1,5}$")) {
                    check = true;
                ...
                if (check) {
                    ...
                    boolean result = dao.insertBook(dto);
                    if (result) {
                        url = SUCCESS;
                    }
                }
            ...
            log("Error at CreatBookServlet " + msg);
            if (msg.contains("duplicate")) {
                errorDTO.setBookIDError("BookID is existed. Please choosse another");
                request.setAttribute("ERROR_BOOK", errorDTO);
            }
        } finally {
